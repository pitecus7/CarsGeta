...
{
    public static EventHandler onMapLoad;
    [Header("Parameters")]
    ...
    public List<Objective> gameModes = new List<Objective>();
    ...
    private void Awake()
    {
        SceneManager.LoadSceneAsync(GameManager.Instance.gameInformation.map.ToString(), LoadSceneMode.Additive).completed += OnMapLoaded;
    }

    private void OnMapLoaded(AsyncOperation obj)
    {
        SetUp();
        onMapLoad(null, null);
    }
    ...
    void SetUp()
    {
        ...
        {
            playerKart.skinnedMeshRenderer.material.color = GameManager.Instance.characters[GameManager.Instance.userInfo.charaterIdSelected].color;
            playerKart.baseStats.Acceleration = GameManager.Instance.characters[GameManager.Instance.userInfo.charaterIdSelected].aceleration;
            playerKart.baseStats.TopSpeed = GameManager.Instance.characters[GameManager.Instance.userInfo.charaterIdSelected].speed;

            if (GameManager.Instance.characters[GameManager.Instance.userInfo.charaterIdSelected].control >= 20)
            {
                playerKart.baseStats.Steer = 3;
            }
            else if (GameManager.Instance.characters[GameManager.Instance.userInfo.charaterIdSelected].control >= 15)
            {
                playerKart.baseStats.Steer = 8;
            }
            else
            {
                playerKart.baseStats.Steer = 10;
            }

            gameModes.ForEach(each =>
            {
                if (each.gameMode == GameManager.Instance.gameInformation.gameMode)
                {
                    Instantiate(each.gameObject, transform);
                }
            });
        }
